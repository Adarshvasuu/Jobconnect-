...
export const MainMenuPage = () => {
  ...
  useEffect(() => {
    ...
    profileApi
      .getProfile()
      .then((d) => {
        if (isMounted && d?.profile) setProfile(d.profile);
      })
      .catch(() => {});
    ...
    applicationApi
      .getMyApplications()
      .then((d) => {
        if (isMounted && d?.applications && d.applications.length > 0) {
  ...
  return (
    <div className="page-wrapper" style={{ backgroundColor: 'transparent', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <Navbar />

      <main className="main-content" style={{ flex: 1, padding: '24px 0 64px' }}>
        <div className="container animate-fade-in" style={{ maxWidth: '1240px', margin: '0 auto', padding: '0 20px' }}>
          
          {/* 1. Centered JobConnect Signature & Welcome Section */}
          <section style={{ textAlign: 'center', padding: '20px 0 32px', position: 'relative' }}>
            {/* Animated Signature Wordmark in the Dead Center */}
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '12px' }}>
              <Signature text="JobConnect" color="#1D4ED8" fontSize={58} delay={0.05} duration={1.0} />
            </div>

            <h1 style={{ fontSize: '2.1rem', fontWeight: 900, color: '#0F172A', margin: '0 0 6px', letterSpacing: '-0.02em' }}>
              Welcome to the <span style={{ color: '#2563EB' }}>JobConnect Main Menu</span>
            </h1>
            <p style={{ color: '#475569', fontSize: '1.05rem', fontWeight: 500, margin: '0 auto', maxWidth: '640px' }}>
              Verified career opportunities, instant skill matching, and AI recruitment pipelines.
            </p>

            {/* 2. Naukri-Style Unified 3-Segment Search Bar */}
            ...
            {/* 3. Quick Category Discovery Pills */}
            <div
              style={{
                display: 'flex',
                alignItems: 'center',
          ...
          >
            {/* Left Column: Readiness, Resume, Skills & Badges */}
            ...
            {/* Right Column: Pipeline, Saved Jobs & Top Companies */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <InterviewCard />
